...
//WP Email Customisations
//Disable media comments toggle
...
add_action('admin_init', function () {
    ...
    // Register option
    register_setting('brighter_tweaks', 'brighter_disable_media_comments', [
        'type' => 'boolean',
    ...
    add_settings_section(
        'brighter_comment_settings',
        'Comment Settings',
        function () {
            echo '<p>Control comment behaviour for attachments and system messages.</p>';
        },
        'brighter_tweaks'
    );

    // Disable media comments toggle
    add_settings_field('brighter_disable_media_comments', 'Disable Comments on Media Attachments', function () {
        $checked = get_option('brighter_disable_media_comments', true) ? 'checked' : '';
        echo '<label><input type="checkbox" name="brighter_disable_media_comments" value="1" ' . $checked . '> Prevent comments on media attachments (but keep them for posts & pages)</label>';
    }, 'brighter_tweaks', 'brighter_comment_settings');
});
